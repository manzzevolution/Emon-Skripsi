...
void SaveToSDCard(String dataStream, String extension){
  ...
  delayProgressWithText2(F("Save to SD"),500,500,0,50);
  createTextCenter(12,"Save to SD Card");
  createTextCenter(24,"File Name :" + textFile + extension);
  SdFile::dateTimeCallback(fileDateTime);
  if(SDCardWriter(textFile ,timeNow + dataStream)){
    delayContinue(500,50,100);
    createTextCenterBig("Success");
    Serial.println(" Done !");
    delay(2000);
  }else {
    delayContinue(500,50,100);
    createTextCenterBig("Failed!");
